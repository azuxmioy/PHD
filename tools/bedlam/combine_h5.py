...
for split in data_splits:
    ...
    with h5py.File(os.path.join(output_folder, split, "combined.h5"), 'w') as h5f:

        with h5py.File(os.path.join(output_folder, split, "anno_smpl.h5"), 'r') as f1:
            for name, item in f1.items():
                h5f.create_dataset(name, data=item[...])

        with h5py.File(os.path.join(output_folder, split, "kp2d_vit.h5"), 'r') as f2:
