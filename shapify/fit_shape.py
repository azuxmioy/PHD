"""Single-image SHAPify command line entry point."""
...
from .fitter import load_run_config, merge_dict
...
def _load_openpose(path: str) -> np.ndarray:
    ...
    if "people" in data:
        ...
        people = [value for value in data.values() if isinstance(value, list)]
        if not people:
            raise ValueError(f"Could not find keypoints in {path}")
        pose = max(people, key=lambda value: np.asarray(value, dtype=np.float32).reshape(-1, 3)[:, 2].sum())
    else:
        pose = data
    return np.asarray(pose, dtype=np.float32).reshape(-1, 3)[:25]
