"""Config-driven SHAPify command line runner for static-subject videos.
...
5. Run the multi-view β + body_pose fitter (``shapify.fitter_video``).
6. Write the same SHAPify outputs as the single-image runner.
...
from .fitter_video import VideoFrame, fit_betas_video
...
def _optimizer_config(config: dict) -> VideoShapeFitConfig:
    return VideoShapeFitConfig(**_merge_dict(asdict(VideoShapeFitConfig()), config.get("optimizer", {})))
...
def apply_cli_overrides(config: dict, args) -> dict:
    ...
    for key in (
        "mass_loss_weight",
        "shoulder_loss_weight",
        "height_loss_weight",
        "beta_reg_weight",
        "body_pose_reg_weight",
        "cam_smooth_weight",
        "orient_smooth_weight",
    ):
        value = getattr(args, key, None)
        if value is not None:
            loss[key] = value
...
def main(argv=None):
    ...
    parser.add_argument("--orient_smooth_weight", type=float)
